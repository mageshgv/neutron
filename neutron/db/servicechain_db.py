...
class SpecNodeAssociation(model_base.BASEV2):
    ...
    spec_id = sa.Column(sa.String(36),
...
class ServiceChainInstance(model_base.BASEV2, models_v2.HasId,
    ...
    name = sa.Column(sa.String(50))
    description = sa.Column(sa.String(255))
    params = sa.Column(sa.String(1024))
...
class ServiceChainDbPlugin(schain.ServiceChainPluginBase,
    ...
    def _get_servicechain_node(self, context, node_id):
        try:
            ...
            raise schain.ServiceChainNodeNotFound(sc_node_id=
                                                  node_id)
    ...
    def _get_servicechain_spec(self, context, spec_id):
        try:
            ...
            raise schain.ServiceChainSpecNotFound(sc_spec_id=
                                                  spec_id)
    ...
    def _get_servicechain_instance(self, context, instance_id):
        try:
            ...
            raise schain.ServiceChainInstanceNotFound(sc_instance_id=
                                                      instance_id)
    ...
    def _make_sc_spec_dict(self, spec, fields=None):
        ...
        res['nodes'] = [sc_onode['node_id']
                        for sc_onode in spec['nodes']]
        return self._fields(res, fields)
    ...
    def _set_nodes_for_spec(self, context, spec_db, nodes_id_list):
        ...
        with context.session.begin(subtransactions=True):
            ...
            spec_db.nodes = []
            for node_id in nodes_id_list:
                assoc = SpecNodeAssociation(spec_id=spec_db.id,
                                            node_id=node_id)
    ...
    def get_servicechain_spec(self, context, servicechain_spec_id,
                              fields=None):
        spec = self._get_servicechain_spec(context, servicechain_spec_id)
