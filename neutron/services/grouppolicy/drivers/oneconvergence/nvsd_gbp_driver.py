# Copyright 2014 OneConvergence, Inc. All Rights Reserved.
#
#    Licensed under the Apache License, Version 2.0 (the "License"); you may
#    not use this file except in compliance with the License. You may obtain
#    a copy of the License at
#
#         http://www.apache.org/licenses/LICENSE-2.0
#
#    Unless required by applicable law or agreed to in writing, software
#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
#    License for the specific language governing permissions and limitations
#    under the License.

from neutron.common import exceptions as n_exc
from neutron.common import log
from neutron.openstack.common import excutils
from neutron.openstack.common import log as logging
from neutron.services.grouppolicy.drivers.oneconvergence import nvsd_gbp_api
from neutron.services.grouppolicy.drivers import resource_mapping as res_map

LOG = logging.getLogger(__name__)


class NvsdGbpDriver(res_map.ResourceMappingDriver):
    """One Convergence NVSD Group Policy Driver for Group Policy Service Plugin

    This class inherits from ResourceMappingDriver and overrides the implicit
    Subnet creation for an EndPointGroup. One Convergence NVSD only supports
    REDIRECT to an L2 Service at present and the Provider and Consumer EPGs
    have to be on the same network and subnet. Hence, One Convergence NVSD
    Group Policy Driver creates only one default L2 Policy for a tenant.
    Further, the EPGs do not have a one-to-one mapping to a subnet, but rather
    multiple EPGs are mapped to one subnet. One Convergence NVSD maps an EPG to
    a NVSD Port Group.
    """
    def __init__(self):
        self.nvsd_api = nvsd_gbp_api.NVSDServiceApi()

    @log.log
    def create_endpoint_postcommit(self, context):
        super(NvsdGbpDriver, self).create_endpoint_postcommit(context)
        try:
            self.nvsd_api.create_endpoint(context._plugin_context,
                                          context.current)
        except Exception:
            with excutils.save_and_reraise_exception():
                super(NvsdGbpDriver, self).delete_endpoint_postcommit(context)

    @log.log
    def update_endpoint_postcommit(self, context):
        self.nvsd_api.update_endpoint(context._plugin_context,
                                      context.current)

    @log.log
    def delete_endpoint_postcommit(self, context):
        self.nvsd_api.delete_endpoint(context._plugin_context,
                                      context.current['id'])
        super(NvsdGbpDriver, self).delete_endpoint_postcommit(context)

    @log.log
    def create_endpoint_group_precommit(self, context):
        #Reuse the previously created implicit L2 Policy for the tenant
        import pdb;pdb.set_trace()
        if not context.current['l2_policy_id']:
            l2ps = context._plugin.get_l2_policies(
                            context._plugin_context,
                            filters=({'description':
                                      ["Implicitly created L2 policy"],
                                      "tenant_id": [
                                            context.current['tenant_id']]}))
            if l2ps:
                context.set_l2_policy_id(l2ps[0]['id'])

    @log.log
    def create_endpoint_group_postcommit(self, context):
        subnets = context.current['subnets']
        if subnets:
            l2p_id = context.current['l2_policy_id']
            l2p = context._plugin.get_l2_policy(context._plugin_context,
                                                l2p_id)
            l3p_id = l2p['l3_policy_id']
            l3p = context._plugin.get_l3_policy(context._plugin_context,
                                                l3p_id)
            router_id = l3p['routers'][0]
            for subnet_id in subnets:
                self._use_explicit_subnet(context, subnet_id, router_id)
        else:
            self._use_implicit_subnet(context)
        import pdb;pdb.set_trace()
        self._handle_contracts(context)

        try:
            self.nvsd_api.create_endpointgroup(context._plugin_context,
                                               context.current)
            for contract_id in context.current.get('consumed_contracts'):
                contract = self.nvsd_api.get_contract(context._plugin_context,
                                                      contract_id)
                left_group = contract.get('left_group', [])
                left_group.append(context.current['id'])
                body = {'id': contract_id,
                        'tenant_id': contract['tenant_id'],
                        'left_group': left_group}
                self.nvsd_api.update_contract(context._plugin_context, body)
            for contract_id in context.current.get('provided_contracts'):
                contract = self.nvsd_api.get_contract(context._plugin_context,
                                                      contract_id)
                right_group = contract.get('right_group', [])
                right_group.append(context.current['id'])
                body = {'id': contract_id,
                        'tenant_id': contract['tenant_id'],
                        'right_group': right_group}
                self.nvsd_api.update_contract(context._plugin_context, body)
        except Exception:
            with excutils.save_and_reraise_exception():
                self.delete_endpoint_group_postcommit(context)

    @log.log
    def update_endpoint_group_postcommit(self, context):
        import pdb;pdb.set_trace()
        super(NvsdGbpDriver, self).update_endpoint_group_postcommit(context)
        self.nvsd_api.update_endpointgroup(context._plugin_context,
                                           context.current)
        #TODO(Magesh): Fetch list of contracts having endpointgroup_id in
        #either left or right groups. If the consumed_contracts and
        #provided_contracts are not same as left_group and right_group, then
        #perform an update. For now only supporting add contract to an EPG
        for contract_id in context.current.get('consumed_contracts'):
            contract = self.nvsd_api.get_contract(context._plugin_context,
                                                  contract_id)
            left_group = contract.get('left_group', [])
            left_group.append(context.current['id'])
            body = {'id': contract_id,
                    'tenant_id': contract['tenant_id'],
                    'left_group': left_group}
            self.nvsd_api.update_contract(context._plugin_context, body)
        for contract_id in context.current.get('provided_contracts'):
            contract = self.nvsd_api.get_contract(context._plugin_context,
                                                  contract_id)
            right_group = contract.get('right_group', [])
            right_group.append(context.current['id'])
            body = {'id': contract_id,
                    'tenant_id': contract['tenant_id'],
                    'right_group': right_group}
            self.nvsd_api.update_contract(context._plugin_context, body)

    @log.log
    def delete_endpoint_group_postcommit(self, context):
        self.nvsd_api.delete_endpointgroup(context._plugin_context,
                                           context.current['id'])
        for contract_id in context.current.get('consumed_contracts'):
            contract = self.nvsd_api.get_contract(context._plugin_context,
                                                  contract_id)
            left_group = contract.get('left_group', [])
            if left_group:
                left_group.remove(context.current['id'])
                body = {'id': contract_id,
                        'tenant_id': contract['tenant_id'],
                        'left_group': left_group}
                self.nvsd_api.update_contract(context._plugin_context, body)
        for contract_id in context.current.get('provided_contracts'):
            contract = self.nvsd_api.get_contract(context._plugin_context,
                                                  contract_id)
            right_group = contract.get('right_group', [])
            if right_group:
                right_group.remove(context.current['id'])
                body = {'id': contract_id,
                        'tenant_id': contract['tenant_id'],
                        'right_group': right_group}
                self.nvsd_api.update_contract(context._plugin_context, body)
        l2p_id = context.current['l2_policy_id']
        l2p = context._plugin.get_l2_policy(context._plugin_context, l2p_id)
        l3p_id = l2p['l3_policy_id']
        l3p = context._plugin.get_l3_policy(context._plugin_context, l3p_id)
        router_id = l3p['routers'][0]
        for subnet_id in context.current['subnets']:
            self._cleanup_subnet(context, subnet_id, router_id)
        

    @log.log
    def create_l2_policy_postcommit(self, context):
        super(NvsdGbpDriver, self).create_l2_policy_postcommit(context)

    @log.log
    def delete_l2_policy_postcommit(self, context):
        pass

    @log.log
    def create_policy_classifier_postcommit(self, context):
        self.nvsd_api.create_policy_classifier(context._plugin_context,
                                               context.current)

    @log.log
    def update_policy_classifier_postcommit(self, context):
        self.nvsd_api.update_policy_classifier(context._plugin_context,
                                               context.current)

    @log.log
    def delete_policy_classifier_postcommit(self, context):
        self.nvsd_api.delete_policy_classifier(context._plugin_context,
                                               context.current['id'])

    @log.log
    def create_policy_action_postcommit(self, context):
        self.nvsd_api.create_policy_action(context._plugin_context,
                                           context.current)

    @log.log
    def update_policy_action_postcommit(self, context):
        self.nvsd_api.update_policy_action(context._plugin_context,
                                           context.current)

    @log.log
    def delete_policy_action_postcommit(self, context):
        self.nvsd_api.delete_policy_action(context._plugin_context,
                                           context.current['id'])

    @log.log
    def create_policy_rule_postcommit(self, context):
        self.nvsd_api.create_policy_rule(context._plugin_context,
                                         context.current)

    @log.log
    def update_policy_rule_postcommit(self, context):
        self.nvsd_api.update_policy_rule(context._plugin_context,
                                         context.current)

    @log.log
    def delete_policy_rule_postcommit(self, context):
        self.nvsd_api.delete_policy_rule(context._plugin_context,
                                         context.current['id'])

    @log.log
    def create_contract_postcommit(self, context):
        super(NvsdGbpDriver, self).create_contract_postcommit(context)
        self.nvsd_api.create_contract(context._plugin_context,
                                      context.current)

    @log.log
    def update_contract_postcommit(self, context):
        super(NvsdGbpDriver, self).update_contract_postcommit(context)
        self.nvsd_api.update_contract(context._plugin_context,
                                      context.current)

    @log.log
    def delete_contract_postcommit(self, context):
        super(NvsdGbpDriver, self).delete_contract_postcommit(context)
        self.nvsd_api.delete_contract(context._plugin_context,
                                      context.current['id'])

    def _use_explicit_subnet(self, context, subnet_id, router_id):
        interface_info = {'subnet_id': subnet_id}
        try:
            self._add_router_interface(context, router_id, interface_info)
        except n_exc.BadRequest:
            pass

    def _use_implicit_subnet(self, context):
        #One Convergence NVSD does not support REDIRECT to a different Subnet
        #at present. So restricting to use same subnet for a given L2 Policy
        epgs = context._plugin.get_endpoint_groups(context._plugin_context,
                                                   filters=({'l2_policy_id':
                                        [context.current['l2_policy_id']]}))
        for epg in epgs:
            if epg['subnets']:
                context.add_subnet(epg['subnets'][0])
                return
        #Create a new Subnet for first EPG using the L2 Policy
        super(NvsdGbpDriver, self)._use_implicit_subnet(context)

    def _cleanup_subnet(self, context, subnet_id, router_id):
        #Cleanup is performed only when the last EPG on subnet is removed
        epgs = context._plugin.get_endpoint_groups(context._plugin_context)
        for epg in epgs:
            epg_subnets = epg['subnets']
            if subnet_id in epg_subnets:
                return
        super(NvsdGbpDriver, self)._cleanup_subnet(context, subnet_id,
                                                   router_id)
