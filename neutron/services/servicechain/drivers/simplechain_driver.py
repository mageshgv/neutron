...
class SimpleChainDriver(object):
    ...
    def create_servicechain_instance_postcommit(self, context):
        ...
        sc_spec_id = sc_instance.get('spec_id')
        ...
        for sc_node_id in sc_node_ids:
            ...
            template_params = sc_instance.get('params', {})
            stack_param = {}
            ...
            self._insert_chain_stack(context._plugin_context.session,
    ...
    def delete_servicechain_instance_postcommit(self, context):
        stack_ids = self._get_chain_stacks(context._plugin_context.session,
                                           context.current['id'])
        for stack in stack_ids:
            HeatClient(context._plugin_context).stacks.delete(
                stack.stack_id)
        self._delete_chain_stacks(context._plugin_context.session,
                                  context.current['id'])

    def _delete_chain_stacks(self, session, sc_instance_id):
    ...
    def _insert_chain_stack(self, session, sc_instance_id, stack_id):
        with session.begin(subtransactions=True):
